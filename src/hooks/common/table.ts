...
type PaginationParams = Pick<PaginationProps, 'currentPage' | 'pageSize' | 'total'>;
...
export function useTableOperate<TableData>(
  ...
  function handleEdit(id: TableData[keyof TableData], type: UI.TableOperateType = 'edit') {
...
export function defaultTransform<ApiData>(response: {
  data: Api.Common.PaginatingQueryRecord<ApiData> | null;
  error: any;
}): PaginationData<ApiData> {
  const { data, error } = response;

  if (!error && data) {
    const { records, pageIndex, pageSize, totalCount, games, machines, list } = data;

    return {
      data: records || games || machines || list,
      pageIndex,
      pageSize,
      totalCount
    };
