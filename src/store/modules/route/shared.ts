 ...
 *
 * 模板态：不过滤，原样返回（privilege 已 stub/移除）
 */
export function filterAuthRoutesByRoles(routes: ElegantConstRoute[]) {
  return routes.flatMap(route => filterAuthRouteByRoles(route));
}

/**
 * Filter auth route by roles
 */
function filterAuthRouteByRoles(route: ElegantConstRoute): ElegantConstRoute[] {
  const filterRoute = { ...route };

  if (filterRoute.children?.length) {
    filterRoute.children = filterRoute.children.flatMap(item => filterAuthRouteByRoles(item));
  }
